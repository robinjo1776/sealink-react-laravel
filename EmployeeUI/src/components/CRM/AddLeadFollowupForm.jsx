...
const AddLeadFollowupForm = ({ onClose, onAddFollowup }) => {
      ...
      try {
        ...
        if (followupData.id) {
          response = await axios.put(
            `http://127.0.0.1:8000/api/lead-followup/${followupData.id}`,
            formattedFollowupData,
            { headers }
          );
          Swal.fire(
            "Updated!",
            "Follow-up data has been updated successfully.",
            "success"
          );
        } else {
          ...
          Swal.fire(
            "Saved!",
            "Follow-up data has been saved successfully.",
            "success"
          );
        }
        ...
        console.error(
          "Error saving/updating follow-up:",
          error.response ? error.response.data : error.message
        );
        Swal.fire(
          "Error",
          "An error occurred while saving/updating the follow-up.",
          "error"
        );
      }
    } else {
  ...
  const validateLead = () => {
    return (
      followupData.lead_no &&
      followupData.lead_date &&
      followupData.lead_type &&
          ...
          <div className="form-row">
            {followupData.products.map((product, index) => (
              <FollowupProductForm
                ...
                onRemove={() => {
                  const updatedProducts = followupData.products.filter(
                    (_, i) => i !== index
                  );
                  setFollowupData({
                    ...followupData,
